...
const FineManagement = () => {
  ...
  const fetchFines = async () => {
    try {
      ...
      let endpoint = 'http://localhost:3000/api/fines';
      if (statusFilter === 'unpaid') {
        endpoint = 'http://localhost:3000/api/fines/unpaid';
      }
  ...
  const handlePaymentStatus = async (fineId, newStatus) => {
    try {
      setError(null);
      const response = await axios.put(
        `http://localhost:3000/api/fines/${fineId}`,
        {
      ...
      if (response.data) {
        ...
        alert(`Fine successfully marked as ${newStatus}`);
      }
    } catch (err) {
                ...
                <td className="action-buttons">
                  {fine.payment_status === 'unpaid' && (
                    <button
                      ...
                      onClick={() => handlePaymentStatus(fine._id, 'paid')}
                    >
                      Mark as Paid
                    </button>
                  )}
                  {fine.payment_status === 'paid' && (
                    <button
                      className="action-button unpay-btn"
                      onClick={() => handlePaymentStatus(fine._id, 'unpaid')}
                    >
                      Mark as Unpaid
                    </button>
                  )}
